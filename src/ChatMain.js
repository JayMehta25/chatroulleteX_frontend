...
const constructAudioUrl = (audioObj) => {
  ...
  const API_URL = 'https://vibester-backend12.onrender.com';
  if (url.startsWith('/')) {
    url = `${API_URL}${url}`;
  }
...
function ChatMain() {
  const API_URL = 'https://vibester-backend12.onrender.com'; // Updated to new backend base URL
  ...
  const renderMessage = (msg, index) => {
    ...
    if (msg.attachment && !msg.attachment.isLocal && attachmentUrl && attachmentUrl.startsWith('/')) {
            ...
            <div style={{ borderRadius: "10px", overflow: "hidden", marginBottom: 8, minHeight: 40, minWidth: 100, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              {msg.isSending ? (
                ...
                <button
                  ...
                  onClick={() => setModalImageUrl(attachmentUrl)}
                >
                  View Image
                </button>
              )}
          ...
          {msg.attachment && (
            <div className="mt-2" style={{ borderRadius: "10px", overflow: "hidden" }}>
              {mediaType === "video" ? (
                ...
                // Show a 'View Image' button instead of auto-rendering the image
                <button
                  className="btn btn-primary"
                  style={{ margin: '8px 0', padding: '6px 16px', fontWeight: 600 }}
                  onClick={() => {
                    setModalImageUrl(attachmentUrl);
